...
import { defaultData } from './data.js';
      ...
      function renderMapMarkers(){
        ...
        filtered.forEach(place=>{
          ...
          const marker=L.marker([place.lat,place.lng],{icon}).addTo(map).bindTooltip(escapeHtml(place.name),{direction:'top',offset:[0,-25]});
      ...
      function renderHome(){
        ...
        $('#next-event-title').textContent=next?next.title:'下一个值得期待的日子'; $('#next-event-count').textContent=next?`${daysUntil(next.nextDate)===0?'就是今天':`还有 ${daysUntil(next.nextDate)} 天`} · ${new Intl.DateTimeFormat('zh-CN',{month:'long',day:'numeric'}).format(new Date(next.nextDate+'T12:00:00'))}`:'去添加一个重要日子吧';
      ...
      function renderWishList(){
        ...
        $('#wish-list').innerHTML=wishes.length?wishes.map(w=>`<article class="wish-item"><span class="tag">${w.status==='done'?'已完成':'待完成'}</span><h3>${escapeHtml(w.title)}</h3><p>${escapeHtml(w.note||'')}</p><div class="pill-row">${w.category?`<span class="pill">${escapeHtml(w.category)}</span>`:''}${w.targetDate?`<span class="pill">${escapeHtml(w.targetDate)}</span>`:''}</div>${w.status!=='done' ? `<button class="link-btn" data-action="finish-wish" data-id="${w.id}">标记完成 →</button>` : ''}</article>`).join(''):'<p class="empty-note sm">还没有愿望，先写一个吧。</p>';
      }
      ...
      function renderAll(){ fillCities(); renderHome(); renderCalendar(); renderLetters(); renderTimeline(); renderMapMarkers(); }
      ...
      function openLetter(id){ const l=data.letters.find(x=>x.id===id); if(!l)return; openModal(`<div class="modal-head"><div><p class="eyebrow">${escapeHtml(l.to)} · ${dateText(l.date)}</p><h2 id="modal-title">一封信</h2></div><button class="close" data-action="close-modal" aria-label="关闭">×</button></div><article class="modal-letter"><p class="letter-meta">${escapeHtml(l.to)}</p><h3>${escapeHtml(l.title)}</h3><div class="body">${escapeHtml(l.body)}</div><p class="sign">${escapeHtml(l.from||'我')}</p></article>`); }
      function openEventForm(existing=null, date=''){ const e=existing||{}; openModal(`<div class="modal-head"><div><p class="eyebrow">SAVE THE DATE</p><h2 id="modal-title">${existing?'编辑重要日子':'添加重要日子'}</h2></div><button class="close" data-action="close-modal" aria-label="关闭">×</button></div><form id="event-form" data-id="${e.id||''}"><div class="form-grid"><div class="field"><label>日期</label><input name="date" type="date" required value="${e.date||date||formatDate(new Date())}"></div><div class="field"><label>类型</label><select name="type"><option ${e.type==='纪念日'?'selected':''}>纪念日</option><option ${e.type==='生日'?'selected':''}>生日</option><option ${e.type==='旅行'?'selected':''}>旅行</option><option ${e.type==='第一次约会'?'selected':''}>第一次约会</option><option ${e.type==='日常'?'selected':''}>日常</option></select></div></div><div class="field"><label>这一天的名称</label><input name="title" required maxlength="40" value="${escapeHtml(e.title||'')}" placeholder="例如：我们相识的那天"></div><div class="field"><label>写一点说明</label><textarea name="description" maxlength="300" placeholder="这一天发生了什么？">${escapeHtml(e.description||'')}</textarea></div><div class="modal-actions">${existing?'<button type="button" class="btn danger" data-action="delete-event" data-id="'+e.id+'">删除</button>':''}<button type="button" class="btn secondary" data-action="close-modal">取消</button><button type="submit" class="btn primary">保存</button></div></form>`); }
      function openLetterForm(){ openModal(`<div class="modal-head"><div><p class="eyebrow">WORDS FOR YOU</p><h2 id="modal-title">写一封信</h2></div><button class="close" data-action="close-modal" aria-label="关闭">×</button></div><form id="letter-form"><div class="form-grid"><div class="field"><label>日期</label><input name="date" type="date" required value="${formatDate(new Date())}"></div><div class="field"><label>写给谁</label><input name="to" required value="给你"></div></div><div class="field"><label>标题</label><input name="title" required maxlength="50" placeholder="例如：想和你再去一次海边"></div><div class="field"><label>信的内容</label><textarea name="body" required style="min-height:210px" placeholder="慢慢写，不着急。"></textarea></div><div class="field"><label>署名</label><input name="from" value="我"></div><div class="modal-actions"><button type="button" class="btn secondary" data-action="close-modal">取消</button><button type="submit" class="btn primary">收进信箱</button></div></form>`); }
      function openWishForm(){ openModal(`<div class="modal-head"><div><p class="eyebrow">WISH LIST</p><h2 id="modal-title">添加愿望</h2></div><button class="close" data-action="close-modal" aria-label="关闭">×</button></div><form id="wish-form"><div class="field"><label>愿望标题</label><input name="title" required maxlength="40" placeholder="例如：一起看一场演出"></div><div class="field"><label>分类</label><input name="category" maxlength="20" placeholder="旅行 / 演出 / 小事"></div><div class="field"><label>备注</label><textarea name="note" maxlength="220" placeholder="想一起做什么？"></textarea></div><div class="form-grid"><div class="field"><label>目标日期</label><input name="targetDate" type="date"></div><div class="field"><label>完成状态</label><select name="status"><option value="todo">待完成</option><option value="done">已完成</option></select></div></div><div class="modal-actions"><button type="button" class="btn secondary" data-action="close-modal">取消</button><button type="submit" class="btn primary">保存愿望</button></div></form>`); }
      function openCapsuleForm(){ openModal(`<div class="modal-head"><div><p class="eyebrow">TIME CAPSULE</p><h2 id="modal-title">添加时光胶囊</h2></div><button class="close" data-action="close-modal" aria-label="关闭">×</button></div><form id="capsule-form"><div class="field"><label>胶囊标题</label><input name="title" required maxlength="40" placeholder="例如：给周年纪念日的信"></div><div class="field"><label>内容类型</label><input name="type" maxlength="20" placeholder="信件 / 照片 / 小纸条"></div><div class="field"><label>说明</label><textarea name="note" maxlength="220" placeholder="未来打开时想看到什么？"></textarea></div><div class="field"><label>上传照片（可选）</label><input name="photoFile" type="file" accept="image/*"></div><div class="field"><label>打开日期</label><input name="unlockDate" type="date" required></div><div class="modal-actions"><button type="button" class="btn secondary" data-action="close-modal">取消</button><button type="submit" class="btn primary">保存胶囊</button></div></form>`); }
      function openPasswordForm(){ openModal(`<div class="modal-head"><div><p class="eyebrow">PRIVATE CORNER</p><h2 id="modal-title">${data.password?'修改访问密码':'设置访问密码'}</h2></div><button class="close" data-action="close-modal" aria-label="关闭">×</button></div><form id="password-form"><div class="field"><label>新密码</label><input name="password" type="password" required minlength="4" autocomplete="new-password" placeholder="至少 4 位"></div><p class="small-note">原型会把密码保存在当前浏览器。正式上线时请使用安全的账号系统和服务器端加密。</p><div class="modal-actions">${data.password?'<button type="button" class="btn danger" data-action="remove-password">关闭保护</button>':''}<button type="button" class="btn secondary" data-action="close-modal">取消</button><button type="submit" class="btn primary">保存并锁定</button></div></form>`); }
      ...
      function renderPlacePanel(){
        ...
        const photo=p.photos?.find(isDataPhoto), displayPhoto=photo||p.photos?.[0]||'一张照片';
        panel.innerHTML=`<div class="place-photo">${photo?`<img src="${photo}" alt="${escapeHtml(p.name)} 的照片">`:''}<span class="photo-label">${escapeHtml(photoLabel(displayPhoto))}</span></div><span class="place-type">${escapeHtml(p.type)}</span><h3>${escapeHtml(p.name)}</h3><span class="place-date">${escapeHtml(p.city)} · ${dateText(p.date)}</span><p>${escapeHtml(p.intro)}</p><div class="panel-action"><button class="btn secondary tiny" data-action="open-place" data-id="${p.id}">翻开这段回忆 →</button></div>`;
      }
      ...
      async function handleSubmit(e){
        ...
        if(e.target.id==='place-form'){ const f=new FormData(e.target), id=e.target.dataset.id, files=f.getAll('photoFiles').filter(file=>file && file.size); if(files.length>3||files.some(file=>file.size>900000)){alert('请上传最多 3 张、每张不超过 900KB 的照片。');return;} const uploaded=await Promise.all(files.map(readPhoto)); const prior=id?data.places.find(x=>x.id===id):null; const priorUploads=(prior?.photos||[]).filter(isDataPhoto); const labels=f.get('photos').split(/[,，]/).map(x=>x.trim()).filter(Boolean); const item={id:id||'p'+Date.now(),name:f.get('name').trim(),city:f.get('city').trim(),date:f.get('date'),type:f.get('type'),lat:Number(f.get('lat')),lng:Number(f.get('lng')),intro:f.get('intro').trim(),shared:f.get('shared').trim(),her:f.get('her').trim(),mine:f.get('mine').trim(),letter:f.get('letter').trim(),photos:[...priorUploads,...uploaded,...labels]}; if(id)data.places=data.places.map(x=>x.id===id?item:x);else data.places.push(item);activePlaceId=item.id;saveData();closeModal();showToast(id?'地点已更新':'新的地点已钉在地图上'); return; }
        ...
        if(e.target.id==='wish-form'){ const f=new FormData(e.target); data.wishes.push({id:'w'+Date.now(),title:f.get('title').trim(),category:f.get('category').trim(),note:f.get('note').trim(),targetDate:f.get('targetDate')||'',status:f.get('status')==='done'?'done':'todo'});saveData();closeModal();showToast('愿望已记下');return; }
